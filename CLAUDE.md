...
- Classify window state as stable or transient before implementation. Persist user-adjusted stable state such as the main window frame, split visibility and split widths; do not restore transient selections, action bars, loading overlays, or editors.
...
- Changes to the primary reading window or split layout must also be verified across quit and relaunch after moving, live resizing, using the window zoom/tile actions, hiding or resizing each sidebar, changing the active reading tab, and disconnecting a display. Restored windows must remain fully visible on the current screens.
