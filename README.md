...
- **提问式解释**：解释前可以输入自己的疑问；不输入时直接走默认解释，回车也可提交。
- **右侧阅读上下文栏**：在 PDF 旁边查看当前文档的单词和笔记；列表会跟随阅读位置，点击条目可跳回原文选区。
- **划线笔记**：划线时可以写下自己的理解；同一处划线可追加多条想法，形成笔记列表。
...
- 点击「划线」会先出现轻量输入框，可以写下自己的理解、疑问或总结。
- 笔记内容可以留空；留空时只保存划线和原文。
- 如果再次完整选中已有划线，会出现「添加笔记」入口，可以继续追加新的想法。
- 删除笔记时，对应的划线标注也会同步移除。
