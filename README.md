...
| 功能                   | 说明                                                       |
| ---------------------- | ---------------------------------------------------------- |
| 📖 **PDF 阅读**        | PDFKit 渲染，连续滚动，自动恢复上次阅读位置（跨重启）      |
| 📑 **PDF 目录（TOC）** | 左侧栏展示大纲，随阅读进度高亮当前章节并自动滚动           |
| 🖱 **选词操作菜单**    | 划选后弹出贴近选区的菜单：**翻译 / 高亮 / 划线**           |
| 🌐 **语境感知翻译**    | 每个语境独立翻译；气泡展示语境翻译、语境解释、整句译文     |
| 🖊 **原生 PDF 标注**   | 高亮（黄）/ 划线（蓝），支持 Toggle 移除、部分重叠自动合并 |
