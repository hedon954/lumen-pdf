...
| 🌐 **语境感知翻译**    | 每个语境独立翻译；选中短语/句子时自动切换句子翻译模式；LLM 失败时在气泡底部展示具体原因（缺少配置 / 接口报错等）                       |
| 🖊 **原生 PDF 标注**   | 高亮（黄）/ 划线（蓝，同 macOS Preview），支持跨行精确标注、Toggle 移除、部分重叠自动合并；**标注持久化到 PDF 元数据**，重启后自动恢复 |
| 📝 **划线笔记**        | 划线时自动保存笔记；支持搜索、编辑、删除、跳转；删除笔记时同步移除划线；**Markdown 导出**                                              |
...
3. 翻译气泡展示：
   - **单词模式**：单词 · 音标 · 语境翻译 · 语境解释 · 整句译文 · 原文
   - **句子模式**（选中短语/句子）：整句译文，简化布局
4. 点击「保存到单词本」→ PDF 原文自动添加黄色高亮

### 高亮 / 划线笔记

- **高亮**：选中文字 → 操作菜单 → 「**高亮**」（黄色标注）
- **划线笔记**：选中文字 → 操作菜单 → 「**划线笔记**」（蓝色下划线 + 自动保存笔记）
...
└── docs/
    ...
    ├── tdd/tdd-2026-03-22.md   技术实现文档 v1.0.0（总体架构）
    ├── tdd/tdd-2026-03-24.md   技术实现文档 v1.0.2（2026-03-24 迭代）
