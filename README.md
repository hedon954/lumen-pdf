...
- **多轮提问式解释**：解释前可以输入自己的疑问；得到 AI 解释后还能继续追问，并把单条或全部 AI 回复保存为笔记。
- **右侧阅读上下文栏**：在 PDF 旁边查看当前文档的单词和笔记；列表会跟随阅读位置，点击条目可跳回原文选区。
- **划线与笔记分离**：划线可直接落笔不打断阅读；笔记支持 Markdown，同一选区可以沉淀多条想法。
...
| API Key | 存入 macOS Keychain，不写入明文配置文件；本地重装后会继续复用已有配置 |
...
- 得到解释后，新的追问输入框会出现在解释下方；后续问答向下追加，并携带前文上下文。
- 最近 10 轮问答会保留；更早对话会被压缩成摘要，同时原始选中文案始终保留。
- 解释窗口可以拖动和缩放；流式输出只有在你停留在底部时才会持续跟随。
- 每条 AI 回复都可以单独保存为笔记，也可以一次保存全部回复；删除时会删除本次已保存的 AI 回复笔记。
...
- 笔记支持 Markdown；内容可以留空，留空时只保存笔记划线和原文。
...
- 同一选区的多条笔记会分开展示并带上时间；笔记卡片默认收起，可展开查看完整内容。
- 打开或关闭右侧栏时，会尽量保留当前 PDF 页码和页内滚动位置，避免阅读进度回退。

### 管理单词和笔记

切换到“单词本”或“笔记”页，可以搜索、编辑、删除已有内容，也可以跳回原 PDF 位置继续阅读。笔记支持 Markdown 和多条内容，适合对同一段原文逐步追加不同阶段的理解。
