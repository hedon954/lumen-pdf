...
- 单词与跨行句子选区会保留真实 PDFKit 选区，并叠加黄色高亮与轻阴影；翻译浮窗使用不透明圆角卡片和锚定箭头，靠近选区显示，支持拖动且不会透出后方 PDF（[7b42805](https://github.com/hedon954/lumen-pdf/commit/7b4280559e74b8890ff0c651d3f27c9a0a44f3b9)）。
- 顶部统一为拖拽、播放、重试、关闭四个等大紧凑按钮；单词与音标同行，长句原文会完整换行而不截断；宽度、自动高度和窗口高度上限继续沿用既有逻辑（[7b42805](https://github.com/hedon954/lumen-pdf/commit/7b4280559e74b8890ff0c651d3f27c9a0a44f3b9)）。

### 修复

- 浮窗初始位置和拖动后的箭头按根层与浮层局部坐标正确指向选区，避免靠近侧栏或 Inspector 时偏离高亮文字（[f87d008](https://github.com/hedon954/lumen-pdf/commit/f87d008696f93c08f3e997e389ac42f940e7ac80)）。
- 译文 JSON 闭合、收到 `finish_reason` 或 `[DONE]` 后立即结束流式等待；词典音标额外等待最多约 1.5 秒，避免翻译已完成仍一直转圈（[067290d](https://github.com/hedon954/lumen-pdf/commit/067290d8cf251309004c3c6970579b4c98af21d2)）。
...
- Token 与费用页新增最近 26 周的每日调用热点图，支持按模型筛选，并汇总调用次数、Token 与估算费用；调用日志重新整理为更易扫描的状态列表和请求/响应详情，同时移除内部图片能力探测产生的无意义记录（[894881e](https://github.com/hedon954/lumen-pdf/commit/894881e8ae8767a9910321620d09cb7a0c0918ae)）。
- LLM 设置为每个内置服务商提供官方 API Key 申请入口；设置页统一压缩顶部留白、移除重复标题，并把提示词验证结果放到变量说明与编辑器之间，验证按钮通过进度、成功和失败状态动画明确反馈（[894881e](https://github.com/hedon954/lumen-pdf/commit/894881e8ae8767a9910321620d09cb7a0c0918ae)）。
- 翻译浮窗提升到阅读窗口根层，可跨越 PDF、侧栏与检查器自由拖动；保存到单词本或笔记、删除与关闭仍保持原有联动（[894881e](https://github.com/hedon954/lumen-pdf/commit/894881e8ae8767a9910321620d09cb7a0c0918ae)）。

### 修复

- 统一 PDFKit 下划线的 sRGB 红色外观，避免新建、恢复、撤销或重建后显示成黑色；手动拖动浮窗时也能到达窗口四周，不再受自动避让区域限制（[894881e](https://github.com/hedon954/lumen-pdf/commit/894881e8ae8767a9910321620d09cb7a0c0918ae)）。
