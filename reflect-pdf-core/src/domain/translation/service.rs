...
impl TranslationDomainService {
    ...
    pub async fn translate(&self, request: TranslationRequest) -> Result<TranslationResult, ReflectError> {
...
mod tests {
    ...
    impl Translator for FailingLlm {
        async fn translate(&self, _word: &str, _sentence: &str) -> Result<TranslationResult, ReflectError> {
            Err(ReflectError::LlmApiError { message: "timeout".to_string() })
        }
    }
