...
设置里的 Extra Config 是用户看得见、改得了的请求附加字段。关闭 thinking 的厂商字段由系统按服务商写入 Extra Config 默认值；用户没改就用默认，改了就用用户的。不再在请求里偷偷加字段，也不再追加 `/no_think`。编辑器按 json.site 的方式：输入时按层级自动缩进，整段格式化只在点画笔时发生。每个内置服务商在 LLM 配置表单外右下角给出官方申请链接。
...
- Extra Config 使用内置轻量 JSON 编辑器：键、字符串、数字、`true`/`false`/`null` 与括号用不同颜色；左侧行号，当前行高亮。
- 回车按当前括号层级插入缩进；若光标后紧跟 `}` / `]`，同时把闭合括号放到下一行。输入过程中不整段重排、不排序键。
- 工具栏画笔才会把合法 JSON 格式化（缩进、键排序）。失焦或保存不得改写正在编辑的文本排版。
- 不合法时保持原文，方便继续改。
...
4. Extra Config 在 LLM 配置卡片内、模型字段下方；打开时已是格式化后的 JSON，键有语法着色。在 `{` 与 `}` 之间回车会得到缩进行，失焦不会把全文重排。
