...
翻译继续用根层 `ReadingOverlayWindow`，打开 `showsAnchorPointer`、`opaqueChrome` 和 Look Up 定位顺序。卡片与三角用 `textBackgroundColor` 实心填充，不用 vibrancy / `NSPopover`：系统弹出层不能拖动，半透明会透出 PDF。三角单独画在朝向选区的 padding 里，沿边对准选区中线；不要用自定义 Path `clipShape` 整张卡片。流式在 JSON 闭合或 `finish_reason` / `[DONE]` 时结束读取；词典音标查询限时 1.5 秒。
...
- 翻译卡片 `opaqueChrome`；箭头 `alongEdge` 对准选区中线，与选区间隙 2pt。卡片变高被窗口钳制后，箭头仍跟选区 `midY`，不要用系统弹出层那种随窗口高度滑开的喙。
...
- Swift：`TranslationPopoverPresentationTests` 覆盖语言标签、单词 / 句子强调色译文选择、拷贝载荷和转圈消失条件；`ReadingOverlayPlacementTests` 覆盖 Look Up 左侧优先、箭头沿边对准选区，以及高卡片被钳制后箭头仍跟单词中线。
