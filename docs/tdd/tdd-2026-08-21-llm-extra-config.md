...
空字符串 = 未修改 = 用上表。`{}` = 用户关掉默认字段。400 重试只剥 `stream_options`，然后仍合并 Extra Config（含默认或用户值）。后续修订：服务商菜单的「其他」不另写一套 Extra Config；未匹配内置地址时仍走本表，见 [tdd-2026-08-24-llm-provider-other.md](tdd-2026-08-24-llm-provider-other.md)。
